 ...
 * @return string
 */
function mk_agrezer_shop_grid_star_string($rating, $max = 5)
{
    $rating = (float) $rating;
 ...
 */
function mk_agrezer_shop_grid_resolve_orderby($atts)
{
 ...
 * @return array
 */
function mk_agrezer_shop_grid_mini_product_args($atts)
{
...
/**
 * Render callback for mk/products-grid (and legacy mk/agrezer-shop-grid-section).
 *
 ...
 * @return string
 */
function mk_render_agrezer_shop_grid_section($attributes, $content, $block)
{
    if (! class_exists('WooCommerce')) {
        return '<div class="mk-products-grid__wc-notice" style="padding:1rem 1.25rem;border:1px dashed #c3c4c7;background:#fcf9e8;border-radius:4px;font-size:14px;">' .
            esc_html__('WooCommerce must be installed and active for the Products Grid block.', 'mk-builder') .
            '</div>';
    }

    $defaults = array(
        'categoryTitle'           => __('Product Category', 'mk-builder'),
        'productsTitle'           => __('Products', 'mk-builder'),
        'contactTitle'            => __('Contact Info', 'mk-builder'),
    ...
    $orderby = mk_agrezer_shop_grid_resolve_orderby($atts);
    ...
    mk_agrezer_shop_grid_apply_product_orderby($query_args, $orderby);
    ...
    if ($atts['showMiniProductsWidget']) {
        $mini_products = wc_get_products(mk_agrezer_shop_grid_mini_product_args($atts));
    }
    ...
    ?>
    <section
        class="wp-block-mk-agrezer-shop-grid-section agrezer-shop-grid"
        style="<?php echo esc_attr($section_style); ?>"
    >
        <div class="agrezer-shop-grid__container" style="<?php echo esc_attr($container_style); ?>">
            <aside class="agrezer-shop-grid__sidebar" aria-label="<?php esc_attr_e('Shop sidebar', 'mk-builder'); ?>">
                <?php if ($atts['showCategoryWidget']) : ?>
                    <div class="agrezer-shop-widget">
                        ...
                        <?php else : ?>
                            <p class="agrezer-shop-widget__empty"><?php esc_html_e('No categories found.', 'mk-builder'); ?></p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
                ...
                <?php if ($atts['showMiniProductsWidget']) : ?>
                    <div class="agrezer-shop-widget">
                        ...
                        <?php else : ?>
                            <p class="agrezer-shop-widget__empty"><?php esc_html_e('No products to show.', 'mk-builder'); ?></p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            ...
            <div class="agrezer-shop-grid__products-wrap">
                <?php if ($atts['showToolbar']) : ?>
                    <div class="agrezer-shop-grid__toolbar">
                        ...
                        <form method="get" action="<?php echo esc_url(get_permalink()); ?>" class="agrezer-shop-grid__sort-form">
                            <label class="screen-reader-text" for="<?php echo esc_attr($sort_select_id); ?>"><?php esc_html_e('Sort products', 'mk-builder'); ?></label>
                            <select id="<?php echo esc_attr($sort_select_id); ?>" name="orderby" aria-label="<?php esc_attr_e('Sort products', 'mk-builder'); ?>" onchange="this.form.submit()">
                                <option value="default" <?php selected($orderby, 'default'); ?>><?php esc_html_e('Default sorting', 'mk-builder'); ?></option>
                                <option value="date" <?php selected($orderby, 'date'); ?>><?php esc_html_e('Newest', 'mk-builder'); ?></option>
                                <option value="price" <?php selected($orderby, 'price'); ?>><?php esc_html_e('Price: low to high', 'mk-builder'); ?></option>
                                <option value="price-desc" <?php selected($orderby, 'price-desc'); ?>><?php esc_html_e('Price: high to low', 'mk-builder'); ?></option>
                                <option value="popularity" <?php selected($orderby, 'popularity'); ?>><?php esc_html_e('Popularity', 'mk-builder'); ?></option>
                                <option value="rating" <?php selected($orderby, 'rating'); ?>><?php esc_html_e('Average rating', 'mk-builder'); ?></option>
                            </select>
                        </form>
                    </div>
                <?php endif; ?>
                ...
                <div class="agrezer-shop-products" style="<?php echo esc_attr($products_style); ?>">
                        ...
                        while ($products_query->have_posts()) :
                            ...
                            <article class="agrezer-product-card">
                                <?php if ($product->is_on_sale()) : ?>
                                    <span class="agrezer-product-card__badge"><?php esc_html_e('Sale!', 'mk-builder'); ?></span>
                                <?php endif; ?>
                    ...
                    <?php else : ?>
                        <p class="agrezer-shop-grid__empty"><?php esc_html_e('No products found.', 'mk-builder'); ?></p>
                    <?php endif; ?>
                </div>
                ...
                <?php if ($atts['showPagination'] && $products_query->max_num_pages > 1) : ?>
                    ...
                    <?php if (! empty($pagination) && is_array($pagination)) : ?>
                        <nav class="agrezer-shop-grid__pagination" aria-label="<?php esc_attr_e('Product pagination', 'mk-builder'); ?>">
                            <?php
