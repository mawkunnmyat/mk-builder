<?php
/**
 * Mk Updates Section – Hospital News & Updates
 *
...
/**
 * Render callback for mk/updates-section dynamic block.
 ...
 */
function mk_render_updates_section($attributes, $content, $block)
{
    $defaults = array(
        'source'                    => 'recent',
        'categoryId'                => 0,
        ...
        'animationDelay'             => 100,
        'showBlogLink'              => false,
        'blogLinkText'              => __('View All on Blog', 'mk-builder'),
        'blogLinkUrl'               => '',
    );
    ...
    ?>
    <section
        class="updates-section jivaka-section mk-updates-section wp-block-mk-updates-section"
        ...
        >
            ...
            <div class="updates-grid" style="<?php echo esc_attr($grid_style); ?>">
                ...
                if ($updates_query->have_posts()) :
                    $read_more_label = !empty($atts['readMoreLabel']) ? $atts['readMoreLabel'] : __('Read More →', 'mk-builder');
                    $idx = 0;
                    while ($updates_query->have_posts()) :
                        ...
                        <article class="update-post<?php echo esc_attr($stagger); ?>" style="<?php echo $atts['animationOnScroll'] ? 'transition-delay:' . (($idx - 1) * $atts['animationDelay'] / 1000) . 's;' : ''; ?>">
                            ...
                            <div class="update-content">
                                ...
                                <a href="<?php echo esc_url($permalink); ?>" class="update-read-more" rel="bookmark" aria-label="<?php echo esc_attr(sprintf(__('Read more about %s', 'mk-builder'), $title)); ?>">
            ...
            <?php if ($show_blog_link) : ?>
                <?php
                $label = !empty($atts['blogLinkText']) ? $atts['blogLinkText'] : __('View All on Blog', 'mk-builder');
                ?>
