<?php
/**
 * Mk Emergency Units Section – Specialized Emergency Units
 *
 ...
 */
function mk_em_units_register_post_meta()
{
    register_post_meta(
        'post',
        'em_unit_link_text',
        array(
            ...
            'type'         => 'string',
            'default'      => '',
            'description'  => __('Label shown on unit card (e.g. "Level 1 Care", "Golden Hour Ready").', 'mk-builder'),
    ...
    register_post_meta(
        ...
        array(
            ...
            'type'         => 'string',
            'default'      => '',
            'description'  => __('Optional URL for the unit link. Leave empty to use post permalink.', 'mk-builder'),
            'auth_callback' => function () {
                return current_user_can('edit_posts');
            },
        )
...
add_action('init', 'mk_em_units_register_post_meta', 20);

/**
 * Add meta box on post edit screen for Emergency Unit fields.
 */
function mk_em_units_add_meta_box()
{
    add_meta_box(
        'mk_em_unit_meta',
        __('Emergency Unit (for Units Section)', 'mk-builder'),
        'mk_em_units_meta_box_callback',
        'post',
        'normal',
        'default'
    );
}
add_action('add_meta_boxes', 'mk_em_units_add_meta_box');

function mk_em_units_meta_box_callback($post)
{
    wp_nonce_field('mk_em_units_meta', 'mk_em_units_meta_nonce');
    $link_text = get_post_meta($post->ID, 'em_unit_link_text', true);
    $link_url  = get_post_meta($post->ID, 'em_unit_link_url', true);
    ?>
    <p>
        <label for="em_unit_link_text"><strong><?php esc_html_e('Link text', 'mk-builder'); ?></strong></label><br>
        <input type="text" id="em_unit_link_text" name="em_unit_link_text" value="<?php echo esc_attr($link_text); ?>"
               class="widefat" placeholder="<?php esc_attr_e('e.g. Level 1 Care, Golden Hour Ready', 'mk-builder'); ?>">
        <span class="description"><?php esc_html_e('Shown at the bottom of the unit card when this post is used in the Emergency Units section.', 'mk-builder'); ?></span>
    </p>
    <p>
        <label for="em_unit_link_url"><strong><?php esc_html_e('Link URL (optional)', 'mk-builder'); ?></strong></label><br>
        <input type="url" id="em_unit_link_url" name="em_unit_link_url" value="<?php echo esc_attr($link_url); ?>"
               class="widefat" placeholder="<?php esc_attr_e('Leave empty to use post link', 'mk-builder'); ?>">
    </p>
    <?php
}

function mk_em_units_save_meta_box($post_id)
{
    if (!isset($_POST['mk_em_units_meta_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mk_em_units_meta_nonce'])), 'mk_em_units_meta')) {
        return;
    }
...
add_action('save_post', 'mk_em_units_save_meta_box');

/**
 * Render callback for mk/em-units-section dynamic block.
 *
 ...
 */
function mk_render_em_units_section($attributes, $content, $block)
{
    $defaults = array(
        'source'              => 'recent',
        'categoryId'          => 0,
    ...
    ob_start();
    ?>
    <section class="em-section em-units-section mk-em-units-section" style="<?php echo esc_attr($section_style); ?>"
            ...
            <div class="em-triage-grid" style="<?php echo esc_attr($grid_style); ?>">
                ...
                if ($units_query->have_posts()) :
                    ...
                    while ($units_query->have_posts()) :
                        ...
                        $card_label    = sprintf(/* translators: %s: post title */ __('View %s', 'mk-builder'), $title);
                        ?>
                           ...
                           aria-label="<?php echo esc_attr($card_label); ?>">
                            ...
                            <?php else : ?>
                                <div class="em-unit-img" style="<?php echo esc_attr($img_style); ?>background:#e8e8e8;display:flex;align-items:center;justify-content:center;color:#999;font-size:0.9rem;">
                                    <?php esc_html_e('No image', 'mk-builder'); ?>
                                </div>
                            <?php endif; ?>
                    ...
                    $empty_msg = __('No posts found.', 'mk-builder');
                    if (!$has_valid_filter) {
                        if ($atts['source'] === 'category') {
                            $empty_msg = __('Please select a category in the block settings.', 'mk-builder');
                        } elseif ($atts['source'] === 'ids') {
                            $empty_msg = __('Please add specific posts in the block settings.', 'mk-builder');
                        }
                    }
                    ?>
