<?php
/**
 * Mk Awards Section - Uses WordPress Default Posts
 *
 ...
 */
function mk_register_post_award_meta()
{
    register_post_meta(
        'post',
        'award_year',
        array(
            ...
            'description'       => __('Award year override (e.g. 2024). Leave empty to use post date.', 'mk-builder'),
        )
    );
}
add_action('init', 'mk_register_post_award_meta');

/**
 * Add Award Year meta box to Post edit screen (optional override)
 */
function mk_post_award_meta_box()
{
    add_meta_box(
        'mk_award_year',
        __('Award Year (optional)', 'mk-builder'),
        'mk_post_award_year_callback',
        'post',
        'side',
        'default'
    );
}
add_action('add_meta_boxes', 'mk_post_award_meta_box');

function mk_post_award_year_callback($post)
{
    wp_nonce_field('mk_award_year_nonce', 'mk_award_year_nonce');
    $year = get_post_meta($post->ID, 'award_year', true);
    ?>
    <p>
        <label for="award_year"><?php esc_html_e('Year for Awards block (e.g. 2024)', 'mk-builder'); ?></label>
        ...
        <span class="description"><?php esc_html_e('Leave empty to use post date.', 'mk-builder'); ?></span>
    </p>
    <?php
}

function mk_post_save_award_meta($post_id)
{
    if (
        !isset($_POST['mk_award_year_nonce']) ||
        !wp_verify_nonce(
            sanitize_text_field(wp_unslash($_POST['mk_award_year_nonce'])),
            'mk_award_year_nonce'
        )
    ) {
        return;
...
add_action('save_post_post', 'mk_post_save_award_meta');

/**
 * Render callback for mk/awards-section dynamic block.
 ...
 */
function mk_render_awards_section($attributes, $content, $block)
{
    $defaults = array(
        'source'                => 'category',
        'categoryId'            => 0,
    ...
    ?>
    <section
        class="mk-awards-section section-padding awards-section wp-block-mk-awards-section"
