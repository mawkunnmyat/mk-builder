<?php
/**
 * Mk CSR Initiative - Post Meta for Initiatives Section
 *
 ...
 */
function mk_register_csr_icon_meta()
{
    register_post_meta(
        'post',
        'csr_icon_class',
        array(
            ...
            'description'       => __('Font Awesome icon class for CSR Initiatives block (e.g. fas fa-ambulance).', 'mk-builder'),
        )
    );
}
add_action('init', 'mk_register_csr_icon_meta');

/**
 * Add CSR Icon meta box to Post edit screen
 */
function mk_post_csr_icon_meta_box()
{
    add_meta_box(
        'mk_csr_icon',
        __('CSR Initiative Icon (optional)', 'mk-builder'),
        'mk_post_csr_icon_callback',
        'post',
        'side',
        'default'
    );
}
add_action('add_meta_boxes', 'mk_post_csr_icon_meta_box');

function mk_post_csr_icon_callback($post)
{
    wp_nonce_field('mk_csr_icon_nonce', 'mk_csr_icon_nonce');
    $icon = get_post_meta($post->ID, 'csr_icon_class', true) ?: 'fas fa-heart';
    ?>
    <p>
        <label for="csr_icon_class"><?php esc_html_e('Font Awesome class', 'mk-builder'); ?></label>
        ...
        <span class="description"><?php esc_html_e('e.g. fas fa-ambulance, fas fa-heart', 'mk-builder'); ?></span>
    </p>
    <?php
}

function mk_post_save_csr_icon_meta($post_id)
{
    if (
        !isset($_POST['mk_csr_icon_nonce']) ||
        !wp_verify_nonce(
            sanitize_text_field(wp_unslash($_POST['mk_csr_icon_nonce'])),
            'mk_csr_icon_nonce'
        )
    ) {
        return;
...
add_action('save_post_post', 'mk_post_save_csr_icon_meta');

/**
 * Render callback for mk/csr-initiatives-section dynamic block.
 ...
 */
function mk_render_csr_initiatives_section($attributes, $content, $block)
{
    $defaults = array(
        'source'                    => 'category',
        'categoryId'                => 0,
    ...
    ?>
    <section
        class="section-padding mk-csr-initiatives-section wp-block-mk-csr-initiatives-section"
        ...
        >
            ...
            >
                ...
                if ($init_query->have_posts()) :
                    ...
                    $btn_label  = ! empty($atts['readMoreLabel']) ? $atts['readMoreLabel'] : __('View Detail', 'mk-builder');
                    ...
                    if (!$has_valid_filter) {
                        if ($atts['source'] === 'category') {
                            $empty_msg = __('Please select a category in the block settings to display posts.', 'mk-builder');
                        } elseif ($atts['source'] === 'ids') {
                            $empty_msg = __('Please add specific posts in the block settings to display them.', 'mk-builder');
                        }
                    }
                    ?>
                    <div
