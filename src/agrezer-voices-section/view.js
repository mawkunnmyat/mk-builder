...
( function () {
	...
	function initSlider( slider ) {
		...
		var section = slider.closest( '.mk-voices-section' );
		if ( ! section ) {
			return;
		}

		var slides = qsa( slider, '.mk-voices-section__slide' );
		if ( slides.length === 0 ) {
			return;
		}
	...
	function init() {
		document
			.querySelectorAll(
				'.mk-voices-section .mk-voices-section__testimonial[data-voices-slider]'
			)
			.forEach( initSlider );
	}
